...
local function alarm(file, windowText)
  ...
  if not g_resources.fileExists(file) then
    file = "/mods/game_bot/sounds/alarm.ogg"
...
onTextMessage(function(mode, text)
  ...
  if mode == 22 and config.damageTaken.enabled then
    return alarm('/mods/game_bot/sounds/magnum.ogg', "Damage Received!")
  end
    ...
    if alertText:len() > 0 then
      ...
      for i=1,#parts do
        ...
        if text:find(part) then
          return alarm('/mods/game_bot/sounds/magnum.ogg', "Special Message!")
        end
      end
    end
  end
...
onTalk(function(name, level, mode, text, channelId, pos)
  ...
  if mode == 1 and config.defaultMsg.enabled then
    return alarm("/mods/game_bot/sounds/magnum.ogg", "Default Message!")
  end

  if mode == 4 and config.privateMsg.enabled then
    return alarm("/mods/game_bot/sounds/Private_Message.ogg", "Private Message!")
  end
end)
...
macro(100, function()
  ...
  if config.lowHealth.enabled then
    if hppercent() < config.lowHealth.value then
      return alarm("/mods/game_bot/sounds/Low_Health.ogg", "Low Health!")
    end
  end

  if config.lowMana.enabled then
    if hppercent() < config.lowMana.value then
      return alarm("/mods/game_bot/sounds/Low_Mana.ogg", "Low Mana!")
    end
  end
  ...
  for i, spec in ipairs(getSpectators()) do
    if not spec:isLocalPlayer() and not (config.ignoreFriends.enabled and isFriend(spec)) then
      ...
      if config.creatureDetected.enabled then
        return alarm("/mods/game_bot/sounds/magnum.ogg", "Creature Detected!")
      end

      if spec:isPlayer() then
        if spec:isTimedSquareVisible() and config.playerAttack.enabled then
          return alarm("/mods/game_bot/sounds/Player_Attack.ogg", "Player Attack!")
        end
        if config.playerDetected.enabled then
          return alarm("/mods/game_bot/sounds/Player_Detected.ogg", "Player Detected!")
        end
      end
      ...
      if config.creatureName.enabled then
        ...
        for i=1,#fragments do
          ...
          if name:lower():find(frag) then
            return alarm("/mods/game_bot/sounds/alarm.ogg", "Special Creature Detected!")
          end
        end
      end
    end
