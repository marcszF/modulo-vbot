...
local valueInSeconds = function(t)
    local d = 0
    local time = 0
    local cutoff = now - 3000
    
    -- Fast cleanup: removing from front (assuming sorted by time as they are inserted by time)
    -- We use a while loop to remove all expired items from the start
    while #t > 0 do
        local v = t[1]
        if v.t < cutoff then
            table.remove(t, 1)
        else
            -- Since it's sorted, once we hit a valid one, all subsequent are valid
            if time == 0 then
                time = v.t
            end
            -- We can't sum here efficiently without iterating the rest, but we can iterate the rest harmlessly
            -- Or better, we iterate ONLY the valid ones for the sum
            break
        end
    end

    -- Sum remaining valid items
    for _, v in ipairs(t) do
        d = d + v.d
    end

    if time == 0 then return 0 end
    return math.ceil(d/math.max(1, (now-time)/1000))
end
...
macro(500, function()
  ...
  local dmgSum = 0
    ...
    -- Clean up dmgDistribution safely (iterate backwards or use a new table)
    local newDmgDist = {}
    local cutoff = now - 60*1000*10
    
    for i,v in pairs(dmgDistribution) do
      if v.t > cutoff then
        table.insert(newDmgDist, v)
...
onTextMessage(function(mode, text)
  ...
  if not id then
    ...
    if not lackOfData[name] then
      ...
      -- Analyzer] no data for item: "..name.. "inside items.lua -> WasteItems")
    end
