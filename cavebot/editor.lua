...
CaveBot.Editor.registerAction = function(action, text, params)
  ...
  -- Alternate buttons between left and right panels
  CaveBot.Editor.buttonCount = CaveBot.Editor.buttonCount + 1
  local targetPanel
  if CaveBot.Editor.buttonCount % 2 == 1 then
    targetPanel = CaveBot.Editor.ui.buttons.buttonsLeft
  else
    targetPanel = CaveBot.Editor.ui.buttons.buttonsRight
  end

  local button = UI.createWidget('CaveBotEditorButton', targetPanel)
  button:setText(text)
  button:setHeight(20)
  if color then
...
CaveBot.Editor.setup = function()
  ...
  registerAction("Mover para cima", function()
  ...
  registerAction("Mover para Baixo", function()
    local action = CaveBot.actionList:getFocusedChild()
    if not action then return end
    local index = CaveBot.actionList:getChildIndex(action)
  ...
  registerAction("Remover", function()
    local action = CaveBot.actionList:getFocusedChild()
    if not action then return end
    action:destroy()
    CaveBot.save()
